...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try {
    ...
    if (/webp/.test(mime)) {
      ...
      if (isUrl(args[0])) stiker = await sticker(false, args[0], global.packname1, global.author1)
...
handler.help = ['snowm <url>', 'stickernowm <url>']
handler.tags = ['sticker']
handler.command = /^s(tic?ker)?no(wm)?bg$/i
